"""V3 — params_hash provenance, INTEGRATION layer.

The canonical hash + ``g0_confirmed`` matching are pinned in
``tests/test_fingerprint.py`` (the ``agents.fingerprint`` module). This file
pins the wiring that module does NOT cover: that the supervisor computes the
DEPLOYED roster's fingerprints (``deployed_params_hashes``) and that
``evaluate`` actually gates promotion on a matching hash — so a tuned override
can never inherit a G0 earned for different params.
"""
...
# --- deployed roster fingerprints reflect overrides -------------------------
...
def test_agent_fingerprint_captures_overrides():
    ...
    assert config_fingerprint(default) != config_fingerprint(tuned)
    ...
    same = AGENT_FACTORIES["femr_v1"](
        None, {"stop_loss_pct": config_payload(default)["stop_loss_pct"]})
    assert config_fingerprint(same) == config_fingerprint(default)
...
def test_deployed_params_hashes_reflect_overrides(tmp_path):
    ...
    # Every rostered agent's deployed hash == fingerprint(factory(defaults + its
    # override)). (Retired agents whose overrides linger in the json are absent
    # from the roster, so the gate never consults them.)
    for name in hashes:
        override = dict(ov.get(name) or {})
        assert hashes[name] == config_fingerprint(AGENT_FACTORIES[name](conn, override))


# --- evaluate() gates promotion on a matching hash --------------------------
