...
class OICrowdingReversalConfig:
    oi_spike_min: float = 0.10      # fade only when OI grew >= this fraction over the lookback
    z_enter: float = 1.0            # AND |(mid-vwap)/sigma| >= this (the overshoot to fade)
...
class OICrowdingReversalAgent(Agent):
    ...
    def __init__(
        ...
        self.cfg = OICrowdingReversalConfig(
            oi_spike_min=float(c.get("oi_spike_min", 0.10)),
