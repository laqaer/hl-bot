...
def order_rate_ok(
    ...
    *,
    ...
    Counts live order attempts (place / rest / rejected audit rows, is_paper=0)
    ...
    acct = conn.execute(
        """SELECT COUNT(*) FROM agent_decisions
           WHERE action IN ('place','rest','rejected') AND is_paper = 0 AND ts_ms >= ?""",
        (since,),
    ).fetchone()[0]
    ...
    if agent is not None:
        n = conn.execute(
            """SELECT COUNT(*) FROM agent_decisions
               WHERE agent = ? AND action IN ('place','rest','rejected')
