...
# truth, run one consolidated engine cycle (PAPER unless HLBOT_TICK_ARGS opts
# into --live), evaluate guardrails, then self-assess health. Each step is
# best-effort so one transient failure doesn't skip the rest; health runs last
# and pages on trouble.
set -uo pipefail
...
run uv run hlbot run --max-cycles 1 ${HLBOT_TICK_ARGS:-}
